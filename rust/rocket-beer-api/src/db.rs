...
pub fn find_beer_by_id(id: &str, pool: Pool) -> Option<Beer> {
...
pub fn find_all_beer(pool: Pool) -> Option<Vec<Beer>> {
    let conn: &SqliteConnection = &pool.get().unwrap();
    beers::table
        .load::<Beer>(conn)
...
pub fn find_all_beer_by_brewery(id: &str, pool: Pool) -> Option<Vec<Beer>> {
    let conn: &SqliteConnection = &pool.get().unwrap();
    beers::table
...
pub fn find_brewery_by_id(id: &str, pool: Pool) -> Option<Brewery> {
    let conn: &SqliteConnection = &pool.get().unwrap();
...
pub fn find_all_breweries(pool: Pool) -> Option<Vec<Brewery>> {
    let conn: &SqliteConnection = &pool.get().unwrap();
    breweries::table
        .load::<Brewery>(conn)
